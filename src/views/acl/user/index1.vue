<template>
    ...
    <el-card style="margin: 10px 0px;">
        ...
        <el-pagination v-model:current-page="pageNo" v-model:page-size="pageSize" :page-sizes="[5, 7, 9, 11]"
            :background="true" layout="prev, pager, next, jumper,->,sizes,total" :total="total" @current-change="getAllInfo"
            @size-change="handler" />
    </el-card>
...
import { reqUserInfo, reqRemoveUser, reqSelectUser, reqAddOrUpdateUser, reqAllRole ,reqSetUserRole} from '@/api/acl/user/index'
import type { User, Records, AllRole ,SetRoleData} from '@/api/acl/user/type'
...
const getAllInfo = async (page = 1) => {
    ...
    console.log(result)
    if (result.code === 200) {
        const { data: { records, total: _total } } = result
        userArr.value = records
        total.value = _total
    }
}
...
const deleteSelectUser = async () => {
    ...
    if (result.code == 200) {
        ...
        getAllInfo(userArr.value.length > 1 ? pageNo.value : pageNo.value - 1);
    }
}
//分页器下拉菜单的自定义事件的回调
const handler = () => {
...
const setRole = async (row: User) => {
    ...
    console.log(result);
    if (result.code == 200) {
        const { data: { allRolesList, assignRoles } } = result
        //存储全部的职位
        allRole.value = allRolesList;
        //存储当前用户已有的职位
        userRole.value = assignRoles;
        //抽屉显示出来
        drawer1.value = true;
    }
}
